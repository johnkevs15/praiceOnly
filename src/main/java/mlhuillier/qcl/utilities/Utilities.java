...
//        String newTenderAmount = getText(By.xpath(transactionAmount));
    ...
    public static void inputNewLoan(By principalLocator, By loanLocator) throws Exception {
        String principalText = String.valueOf(getText(principalLocator));
        double principal = Double.parseDouble(principalText);
        System.out.println(principalText);
        waitTime(2000);

        double deductionAmount = 100;
        double transactionInput = principal - deductionAmount;
        System.out.println(transactionInput);
        waitTime(2000);

        String newLoan = Double.toString(transactionInput);
        System.out.println("Transaction Amount is = (" + newLoan + ").");
        waitTime(1000);

        click(loanLocator, "New Loan Input Field");
        waitTime(1000);

        WebElement loanField = getWebDriver().findElement(loanLocator);
        JavascriptExecutor jsExecutor = (JavascriptExecutor) getWebDriver();
        jsExecutor.executeScript("arguments[0].value = arguments[1];", loanField, newLoan);
        type(loanLocator, newLoan, "New Loan Amount");
    }
